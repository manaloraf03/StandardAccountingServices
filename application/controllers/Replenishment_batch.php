<?php
	...
	class Replenishment_batch extends CORE_Controller
	{
		...
		function transaction($txn=null){
			switch ($txn) {
				case 'list':
					...
					$batch_id=$this->input->get('i',TRUE);
					...
					$response['data']=$m_journal->get_list(
						...
						journal_info.is_replenished=TRUE AND journal_info.batch_id='.$batch_id.'',
						'journal_info.txn_no,
				...
				case 'report':
					...
					$data['batches']=$m_journal->get_list(
						...
						journal_info.batch_id = '.$batch_id.'',
					...
					$this->load->view('template/replenishment_batch_html',$data);
				break;
			}
		}
	}
